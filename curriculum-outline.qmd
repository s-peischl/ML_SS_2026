...
This outline links to **[afternoon lab exercises](exercises/index.qmd)** (numbered tasks and one solution notebook per teaching day) while weaving a few ideas through the week: **two parallel examples** (simulated **genes** where we know the truth + **Palmer Penguins** as one real table), **what a wrong prediction costs** (thresholds, prevalence), **calibration**, **“correlation is not causation”** when reading importance plots, short **data and model summaries**, **reproducibility**, **fairness-style subgroup checks**, and assignments that reward **honest wording** (especially around SHAP).

## Integrated design principles (built into the week)

- **Two parallel data tracks** — **Synthetic genes** in the original linear / logistic notebooks: we scripted the biology so we know which coefficients *should* matter. **Palmer Penguins** ([data card](data/cards/palmer-penguins.qmd), [catalog](data/index.qmd), dedicated notebooks, **Tuesday’s full modeling walk-through in R** using `tidymodels`; [reading companion](modules/index.qmd#chapter-04-a-reproducible-modeling-workflow) for Chapters **4**, **7**, and **8**): one small real table reused Mon→Thu for plots, regression, classification, and workflows.
...
## <img src="assets/hub-icons/dna.svg" class="hub-inline-icon" alt="" aria-hidden="true" /> Day 1 (Monday): Foundations and Regularization
...
## <img src="assets/hub-icons/code.svg" class="hub-inline-icon" alt="" aria-hidden="true" /> Day 2 (Tuesday): Trees, rpart, and the tidymodels Pipeline
...
2. **Incremental pipeline walkthrough** on penguins ([`day02-tidymodels-walkthrough.qmd`](https://github.com/s-peischl/ML_SS_2026/blob/main/_includes/day02-tidymodels-walkthrough.qmd) — also Module 04)
3. **`tune_grid`** for `tree_depth` and `min_n`; tuning plot, tree plot, light VIP
4. Microbiome lab: same grammar on OTU data (grouped CV)

## <img src="assets/hub-icons/lab.svg" class="hub-inline-icon" alt="" aria-hidden="true" /> Day 4 (Thursday): Models, Preprocessing, Metrics, and Importance
