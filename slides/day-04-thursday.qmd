...
- [Back: Day 2 (Tuesday) deck](day-02-tuesday.html)
...
## Thursday start: `tidymodels` flowchart {.flowchart-slide}
...
## Scenario goal (Part B)

Keep preprocessing fixed and learn **model-family differences** fairly:
- same `recipe`
- same folds
- same metric
- only `spec` changes

## Why this stress matters (Part B)

If we change recipe, folds, and model at the same time, we cannot tell what caused a performance difference. Part B isolates **model family** only.
...
- More depth: [parsnip model reference](https://www.tidymodels.org/find/parsnip/) · compare learners in **Part B** on this deck.
...
Fixed settings here for speed  -  in projects, wrap each spec in **`tune_grid()`** like [Day 2 (Tuesday) pipeline](../slides/day-02-tuesday.html#/part-c-pipeline).
...
From this point on, we keep the same workflow structure and stress one new issue at a time.  
Next stress: **correlated numeric predictors** (PCA inside the recipe).

# Part C  -  Dimension reduction (accuracy only) {#part-c-pca}

## Scenario goal (Part C)

Learn when PCA helps: if numeric predictors are strongly correlated, compress them inside the recipe and compare accuracy with/without PCA on the same folds.

## Why this stress matters (Part C)

High collinearity can make models unstable or redundant. PCA gives a compact representation, but costs interpretability.
...
We keep the PCA lesson fixed and move to a different stress: **missing values**.  
Same workflow, new recipe step family: imputation.

# Part D  -  Missing data (accuracy only) {#part-d-impute}

## Scenario goal (Part D)

Handle missingness without leakage: impute **inside the recipe** so each fold learns imputation from analysis rows only.

## Why this stress matters (Part D)

Dropping rows can bias the dataset; imputing before splits leaks information. We need fold-safe imputation.
...
## Scenario goal (Part E)

Show why accuracy can mislead with a rare class and practice choosing/reporting macro and per-class metrics.

## Why this stress matters (Part E)

When one class is rare, accuracy can hide failure on the minority class.  
This part focuses on macro/per-class metrics and confusion matrices.
...
## Scenario goal (Part F)

Interpret fitted models responsibly: identify influential predictors (VIP/SHAP) without making causal claims.

## Why this stress matters (Part F)

A model can score well but still be hard to justify. Interpretation tools help communication and diagnostics, not causal proof.
...
- **Permutation importance**: shuffle one column, watch metric drop  -  model-agnostic; see the **VIP plot** above and the [SHAP notebook](../notebooks/penguins-shap.html).
