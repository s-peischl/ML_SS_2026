...
# Part A  -  Recap (Tuesday's pipeline) {#part-a-recap}
...
- **`metric_set(accuracy)`** for Parts B–D (see [metrics refresh](#/part-metrics); richer metrics in Part E)
...
# Holdout predictions for metrics / ROC teaching slide (shallow tree — not perfect separation)
tree_metrics_spec <- decision_tree(tree_depth = 1, min_n = 20) |>
  set_engine("rpart") |>
  set_mode("classification")
wf_metrics_demo <- workflow() |>
  add_recipe(rec_base) |>
  add_model(tree_metrics_spec)
set.seed(7)
split_metrics <- initial_split(peng_cls, prop = 0.75, strata = y)
fit_metrics_demo <- fit(wf_metrics_demo, training(split_metrics))
pred_metrics_demo <- augment(fit_metrics_demo, testing(split_metrics))
...
metrics_demo_auc <- pred_metrics_demo |>
  roc_auc_gentoo(truth = y, .pred_Gentoo) |>
  pull(.estimate)
...
print(
  plot_roc_gentoo(
    pred_metrics_demo,
    subtitle = paste0(
      "Holdout stump (depth 1); ROC AUC = ",
      round(metrics_demo_auc, 3)
    )
  )
)
```
...
- **Permutation importance** (shuffle one column, watch metric drop) is model-agnostic — see the [SHAP notebook](../notebooks/penguins-shap.html).
