...
- [Chapter 3  -  Rules and trees](../modules/module-03-trees-and-cultures.qmd)
- [Chapter 4  -  Modeling workflow](../modules/module-04-pipeline.qmd)
...
```{r}
#| label: day02-recipe-pipeline-fig
#| echo: false
#| fig-width: 9
#| fig-height: 2.8
pipeline_df <- tibble(
  step = c("Raw columns", "step_zv()", "step_dummy()", "step_normalize()", "Model-ready table"),
  x = 1:5,
  y = 1
)
ggplot(pipeline_df, aes(x, y, label = step)) +
  ...
  labs(title = "Recipe steps run in order (inside each CV fold)")
```
