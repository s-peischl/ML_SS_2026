...
### Tuning parameters: 
- **`cp`:** 
complexity penalty  -  larger `cp` → simpler tree (see trait block after disease)
- **`minsplit`:** 
The minimum number of data points that must be in a node before rpart will even try to split it further.
- **`maxdepth`:** The maximum number of levels (layers) the decision tree is allowed to grow from top to bottom.
...
- Dataset: [Palmer Penguins data card](../data/cards/palmer-penguins.qmd)  -  Adelie vs Gentoo; **bill + island + sex + year**
...
- Today wqe use **`accuracy`**  -  fraction of correct class labels; fine when classes are balanced and errors cost the same.
