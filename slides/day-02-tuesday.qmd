...
- [Back: Day 1 (Monday) deck](day-01-monday.qmd)
- [Next: Day 4 (Thursday) deck](day-04-thursday.qmd)
...
- **`control`:** `maxdepth`, `cp`, `minsplit` — same levers you will tune as **`tree_depth`** and **`min_n`** in `parsnip`
...
- **Today:** classification tree on **all genes**
...
**Gene × trait — regression trees (`rpart`)** on the same simulation as Monday ([linear-regression-lasso.Rmd](../notebooks/linear-regression-lasso.Rmd)): continuous **trait** from **GeneA** and **GeneB**.
...
- Notebook loop over **`cp`** values: [linear-regression-lasso.Rmd](../notebooks/linear-regression-lasso.Rmd)
...
- Real data: [Palmer Penguins](../dataset-penguins.qmd) — Adelie vs Gentoo; **bill + island + sex + year**
