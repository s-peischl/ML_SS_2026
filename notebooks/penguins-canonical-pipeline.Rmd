...
wf_glm <- workflow() |> add_recipe(rec) |> add_model(glm_spec)
wf_rf <- workflow() |> add_recipe(rec) |> add_model(rf_spec)
```

## Cross-validation (`fit_resamples`)

Same **`folds`** and **`metrics`** for each model — run **`fit_resamples()`** once per workflow:

```{r}
set.seed(7)
rs_tree <- fit_resamples(wf_tree, folds, metrics = metrics)
rs_glm <- fit_resamples(wf_glm, folds, metrics = metrics)
rs_rf <- fit_resamples(wf_rf, folds, metrics = metrics)
```
...
```{r}
cmp <- bind_rows(
  collect_metrics(rs_tree) |> mutate(model = "decision_tree"),
  collect_metrics(rs_glm) |> mutate(model = "logistic_glm"),
  collect_metrics(rs_rf) |> mutate(model = "random_forest")
)
