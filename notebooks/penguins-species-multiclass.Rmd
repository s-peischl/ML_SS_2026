...
Here we treat **`species`** as a **three-level** outcome (Adelie, Chinstrap, Gentoo) using **multinomial logistic regression** (`nnet::multinom`) and a **multiclass classification tree** (`rpart`). This complements the **binary** Adelie-vs-Gentoo notebook (`penguins-classification.Rmd`).
...
```{r}
set.seed(24)
tr_idx <- caret::createDataPartition(pg$species, p = 0.75, list = FALSE)[, 1]
train <- pg[tr_idx, ]
test <- pg[-tr_idx, ]
```
...
caret::confusionMatrix(pred_multi, test$species)
```

```{r fig.width=5.5, fig.height=4.5}
cm <- as.data.frame(caret::confusionMatrix(pred_multi, test$species)$table)
...
caret::confusionMatrix(pred_t, test$species)
```
