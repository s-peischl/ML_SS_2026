...
Instructors: a live screenshot of your own layout works well here.
...
Optional: **Positron** from [posit.co/products/positron](https://posit.co/products/positron/) if your course standardizes on it.
...
## ggplot2: one sentence

**Data** + **aesthetic mappings** (`aes`: x, y, colour, …) + **geometric layers** (`geom_point`, `geom_smooth`, …) + **`labs()`** + **`theme_*()`**, built with **`+`**.
...
Course **`.Rmd`** notebooks use the same R + **ggplot2** ideas; the hub renders them with **Quarto**. After today’s slides, open any notebook from the [notebook index](../notebooks/index.qmd) and try **Render** in RStudio.
